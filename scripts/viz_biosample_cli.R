...
# Append a canonical 'universal_source' column derived from all search columns
if (length(search_cols) > 0) {
  df_exp <- df_exp %>%
    mutate(
      .source_text = do.call(paste, c(across(all_of(search_cols)), sep = " ")),
      .source_text = tolower(stringr::str_squish(.source_text)),
      universal_source = dplyr::case_when(
        stringr::str_detect(.source_text, regex("urine|urinary|urinary tract|uti", ignore_case = TRUE)) ~ "urine",
        stringr::str_detect(.source_text, regex("blood|bacteraem|bacterem|sept(ic)?", ignore_case = TRUE)) ~ "blood",
        stringr::str_detect(.source_text, regex("faec|feces|faeces|stool", ignore_case = TRUE)) ~ "feces",
        stringr::str_detect(.source_text, regex("food|meat|poultry|milk|dairy|fish|produce", ignore_case = TRUE)) ~ "food",
        stringr::str_detect(.source_text, regex("env|environment|water|wastewater|sewage|soil|surface|air", ignore_case = TRUE)) ~ "environment",
        is.na(.source_text) | .source_text == "" ~ "unspecified",
        TRUE ~ "others"
      )
    ) %>%
    select(-.source_text)
} else {
  df_exp <- df_exp %>% mutate(universal_source = NA_character_)
}
